...
// Supabase envs (must be valid https URLs + anon key)
...
// Accept totals/macros from different response styles
...
function normalizeAnalyzeResponse(json) {
  ...
  const computedKcal = total_kcal > 0 ? total_kcal : items.reduce((s, it) => s + num(it.kcal), 0);
...
export default function App() {
  ...
  const addToHistory = async (scan) => {
    const normalized = normalizeAnalyzeResponse(scan);
    const entry = {
      id: `${Date.now()}`,
      createdAt: nowISO(),
      total_kcal: round1(normalized?.total_kcal),
      totals: normalized?.totals || { protein_g: 0, carbs_g: 0, fat_g: 0 },
      items: normalized?.items || [],
      photoUri: normalized?.photoUri || null,
      source: normalized?.source || "photo",
  ...
  const analyze = async () => {
    ...
    try {
      ...
      const json = normalizeAnalyzeResponse(JSON.parse(text));
      const enriched = { ...json, photoUri, source: "photo" };
  ...
  const onBarcodeScanned = async ({ data }) => {
    ...
    // cooldown: ignore duplicates within 3 seconds
    const now = Date.now();
    const last = lastBarcodeRef.current;
    if (last.value === code && now - last.at < 3000) return;
    lastBarcodeRef.current = { value: code, at: now };
    ...
    try {
      ...
      const json = normalizeAnalyzeResponse(JSON.parse(text));
      const enriched = { ...json, source: "barcode", barcode: code };

      setResult(enriched);
      await addToHistory(enriched);

      // optional: auto-exit barcode mode after success
      setMode("photo");
    } catch (e) {
  ...
  // If Supabase is configured, show login screen when not logged in
  if (supabase && !session) {
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: "#000" }}>
        ...
        >
          ...
          <Text style={{ color: "#aaa", marginTop: 6 }}>Login to save scans across devices</Text>
          ...
          <Text style={{ color: "#777", marginTop: 12, fontSize: 12 }}>
            If your email confirm link opens localhost, we’ll fix Supabase redirect URLs later.
          </Text>
        </KeyboardAvoidingView>
      </SafeAreaView>
    );
      ...
      <SafeAreaView style={{ backgroundColor: "#000" }}>
        <View style={styles.header}>
          <Text style={styles.headerTitle}>
            Kcal Scan {mode === "barcode" ? "• Barcode" : ""}
          </Text>
          <View style={{ flexDirection: "row", gap: 8 }}>
            <TouchableOpacity
              onPress={() => setMode(mode === "barcode" ? "photo" : "barcode")}
              style={styles.smallBtn}
            >
              <Text style={styles.smallBtnText}>
                {mode === "barcode" ? "📷 Photo mode" : "📦 Scan barcode"}
              </Text>
            </TouchableOpacity>
      ...
      <View style={styles.cameraWrap}>
        ...
        {mode === "barcode" ? (
          <View style={styles.barcodeOverlay}>
            <Text style={styles.barcodeOverlayText}>
              Point camera at barcode…
            </Text>
            <Text style={styles.barcodeOverlaySub}>
              (EAN/UPC works best. Hold steady ~20–30cm)
            </Text>
          </View>
        ) : null}
      </View>
      ...
      <View style={styles.panel}>
        ...
        {mode === "photo" ? (
          ...
          <View style={styles.row}>
            <TouchableOpacity
              onPress={() => {
                // reset barcode cooldown if needed
                lastBarcodeRef.current = { value: null, at: 0 };
                Alert.alert("Barcode mode", "Scan a barcode to fetch USDA branded nutrition.");
              }}
              style={[styles.btn, styles.primary]}
              disabled={loading}
            >
              <Text style={styles.btnText}>
                {loading ? "Fetching..." : "📦 Waiting for scan…"}
              </Text>
            </TouchableOpacity>
          </View>
        )}
        ...
        {loading ? (
          <View style={styles.loadingRow}>
            ...
            <Text style={{ color: "white", marginLeft: 10 }}>
              {mode === "barcode" ? "Looking up USDA…" : "Estimating with AI + USDA…"}
            </Text>
          </View>
        ) : null}
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Text style={{ color: "#aaa", marginTop: 8 }}>No scans yet.</Text>
          }
          renderItem={({ item }) => {
...
const styles = StyleSheet.create({
  ...
  barcodeOverlayText: { color: "white", fontWeight: "900", fontSize: 16 },
  barcodeOverlaySub: { color: "#ddd", marginTop: 4, fontSize: 12 },
