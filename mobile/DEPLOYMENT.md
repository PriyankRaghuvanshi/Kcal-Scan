...
1. Bump **`app.json`** → `"version"` (e.g. `1.0.7` → `1.0.8`).
2. Bump **`eas.json`** → `build.production.cache.key` (e.g. `production-v4-…` → new suffix) **or** pass **`--clear-cache`** once.
3. Build:
...
- **`Build 1.0.7 · <number>`** — `1.0.7` comes from `app.json`; the second token is the **native build** from the store binary.
