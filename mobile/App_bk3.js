...
// Supabase env (mobile/.env)
const SUPABASE_URL = process.env.EXPO_PUBLIC_SUPABASE_URL;
const SUPABASE_ANON_KEY = process.env.EXPO_PUBLIC_SUPABASE_ANON_KEY;

if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
  throw new Error(
    "Missing Supabase env vars. Set EXPO_PUBLIC_SUPABASE_URL and EXPO_PUBLIC_SUPABASE_ANON_KEY in mobile/.env then restart Expo with -c."
  );
}
...
// Accept macros from different shapes
...
// Normalize analyze response (photo)
function normalizeAnalyzeResponse(json) {
  ...
  if (totals.protein_g === 0 && totals.carbs_g === 0 && totals.fat_g === 0 && items.length > 0) {
    totals = items.reduce(
      (acc, it) => ({
...
// Normalize barcode response (GET /barcode/{code})
function normalizeBarcodeResponse(json) {
  // expected:
  // { barcode, name, brand, per_100g:{kcal,protein_g,carbs_g,fat_g}, source, cached }
  const kcal100 = num(json?.per_100g?.kcal);
  const macros100 = {
    protein_g: num(json?.per_100g?.protein_g),
    carbs_g: num(json?.per_100g?.carbs_g),
    fat_g: num(json?.per_100g?.fat_g),
  };
  ...
  return {
    ...
    items: [
      {
        name: json?.name ?? "Barcode product",
        grams: 100,
        confidence: 1,
        kcal: kcal100,
        macros: macros100,
...
export default function App() {
  ...
  const analyzePhoto = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/analyze`, {
        method: "POST",
        body: form, // IMPORTANT: no manual Content-Type
      });
      ...
      Alert.alert("API Error", err?.message ?? "Unknown error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const barcodeLookup = async (code) => {
    ...
    try {
      ...
      Alert.alert("Barcode lookup failed", e?.message ?? "Unknown error");
    } finally {
      setLoading(false);
      setScannedOnce(true);
      // allow scan again after a short delay
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: "#000" }}>
        ...
        >
          ...
          <Text style={{ color: "#777", marginTop: 12, fontSize: 12 }}>
            Email confirm link redirect can be fixed later in Supabase Auth URL settings.
          </Text>
        </KeyboardAvoidingView>
      </SafeAreaView>
    );
    ...
    <View style={styles.container}>
      ...
      <SafeAreaView style={{ backgroundColor: "#000" }}>
        ...
        <View style={styles.modeRow}>
          ...
          <TouchableOpacity
            style={[styles.modeBtn, mode === "barcode" ? styles.modeBtnActive : null]}
            onPress={() => {
              setMode("barcode");
              setPhotoUri(null);
              setResult(null);
          ...
          >
            <Text style={styles.modeText}>🏷️ Barcode</Text>
          </TouchableOpacity>
        </View>
      </SafeAreaView>
      ...
      <View style={styles.cameraWrap}>
        <CameraView
          ...
          onBarcodeScanned={
            mode === "barcode" && !scannedOnce
              ? ({ data }) => barcodeLookup(data)
              : undefined
          }
        />
        ...
        {mode === "barcode" ? (
          <View style={styles.barcodeHint}>
            ...
            <Text style={{ color: "#ccc", marginTop: 4, fontSize: 12 }}>
              We’ll lookup via OpenFoodFacts and cache it in Supabase (per 100g).
            </Text>
          </View>
        ) : null}
      </View>
      ...
      <View style={styles.panel}>
          ...
          <View style={[styles.row, { marginTop: 10 }]}>
            <TouchableOpacity
              onPress={() => {
                ...
                setSelected(null);
                setScannedOnce(false);
                Alert.alert("Ready", "Point the camera at a barcode to scan.");
              }}
              style={[styles.btn, styles.primary]}
              disabled={loading}
            >
              <Text style={styles.btnText}>{loading ? "Looking up..." : "✅ Ready to scan"}</Text>
            </TouchableOpacity>
          </View>
        )}
        ...
        {display ? (
          <View style={styles.resultBox}>
            ...
            {display?.source === "barcode" ? (
              <Text style={styles.badge}>BARCODE</Text>
            ) : (
              <Text style={styles.badge}>PHOTO</Text>
            )}
            ...
            {display?.source === "barcode" && display?.barcode ? (
              <Text style={{ color: "#9aa", marginTop: 8, fontSize: 12 }}>
                Barcode: {display.barcode} • Source: {display?.meta?.provider ?? "openfoodfacts"} • Cached:{" "}
                {String(display?.meta?.cached ?? false)}
              </Text>
            ) : null}
          </View>
        ) : null}
              ...
              >
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.historySub}>
                    P {round1(item?.totals?.protein_g)}g • C {round1(item?.totals?.carbs_g)}g • F{" "}
                    {round1(item?.totals?.fat_g)}g
                  </Text>
                  <Text style={styles.historySub2}>
                    {label}
...
const styles = StyleSheet.create({
  ...
  modeText: { color: "white", fontWeight: "800" },
  ...
  resultTitle: { color: "#aaa", fontWeight: "700", marginBottom: 6 },
  badge: { color: "#7aa7ff", fontSize: 12, fontWeight: "900", marginBottom: 6 },
