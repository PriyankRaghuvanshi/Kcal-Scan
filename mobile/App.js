...
export default function App() {
      ...
      try {
        if (isDeviceHealthAvailable()) {
          ...
          await new Promise((resolve) =>
            initDeviceHealth(() => resolve(), { force: userChanged })
          );
        }
  ...
  useEffect(() => {
    const sub = AppState.addEventListener("change", (next) => {
      if (next === "active") {
        ...
        const uid = userId || session?.user?.id;
        if (uid && isDeviceHealthAvailable()) {
          void (async () => {
            try {
              await new Promise((resolve) =>
                initDeviceHealth(() => resolve(), { force: true })
              );
              const hc = (await getDailyHealthContext(localDayISO())) || {};
              setCoachRecoveryBadge(computeRecoveryModeBadge(hc));
            } catch {
  ...
  async function sendCoachChatTurn() {
    ...
    try {
      ...
      try {
        if (isDeviceHealthAvailable()) {
          await new Promise((resolve) => initDeviceHealth(() => resolve()));
        }
