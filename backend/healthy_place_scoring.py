...
HEALTHY_PLACE_SCORING_VERSION = "v2_menu_distribution"
...
MENU_DISTRIBUTION_SIGNAL_WEIGHTS: Dict[str, float] = {
    "top_3_item_average": 0.32,
    "percent_items_under_600_kcal": 0.20,
    "percent_items_over_30g_protein": 0.20,
    "percent_items_flagged_cut_friendly": 0.16,
    "menu_health_density": 0.12,
}

MENU_DOMINANCE_BY_SOURCE: Dict[str, float] = {
    "real_menu": 0.78,
    "menu_intelligence_store": 0.68,
    "structured_menu": 0.64,
    "llm_inferred": 0.46,
    "heuristic": 0.30,
}
...
def _score_menu_distribution(menu_items: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    item_scores = [float(_clamp(_safe_float(row.get("item_score"), 0.0), 0.0, 100.0)) for row in rows]
    ...
    for row in rows:
        ...
        score = float(_clamp(_safe_float(row.get("item_score"), 0.0), 0.0, 100.0))
...
def score_healthy_place(
    ...
    if bool(menu_signals.get("available")):
        ...
        menu_dominance = _clamp(menu_dominance, 0.20, 0.86)
    ...
    health_score = round(_clamp(blended_score_100 / 10.0, 1.0, 10.0), 1)
